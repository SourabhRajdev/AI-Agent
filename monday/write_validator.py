...
Three layers:
  1. Field resolution  — any representation → column ID via field_resolver
...
All lookups are column-ID-based so the validator works regardless of
whether the caller used a semantic key, a human title, or a raw column ID.
"""

import difflib
import logging

from monday.schema_loader import get_column_types, get_valid_labels
...
def validate_write(values_to_set: dict, board: str) -> list[str]:
    ...
    Resolves each field name to a column ID first, so any representation
    ("Assigned AE", "assigned ae", "AE", "text_mm1rrhc0", etc.) is handled
    transparently before type and label validation.

    Args:
        values_to_set: {field_name: value} from entities.values_to_set
        board:         "sales" | "artists" | "staff"

    Returns:
        List of human-readable error strings.  Empty list = valid.
        Returns [] if schema not loaded (lets Monday.com handle it).
    """
    ...
    if not column_types:
        logger.warning("validate_write: column_types empty for board=%s — skipping", board)
        return []
    ...
    for field, value in values_to_set.items():
        ...
        # ── Field resolution ───────────────────────────────────────────────
        col_id = resolve_to_column_id(field, board)
        ...
        if col_type in _NUMERIC_TYPES:
            try:
                ...
                errors.append(
                    f"'{field}' expects a number, got '{value}'. Example: 2500"
                )
            continue
        ...
        if col_type in _LABEL_TYPES:
            ...
            if labels and str_val.lower() not in labels:
